...
inits <- list() 
inits[[1]] <- list(mu = X_bar - X_sd)
inits[[2]] <- list(mu = X_bar)
inits[[3]] <- list(mu = X_bar + X_sd)
...
# with 5000 samples
samps_mod1_5k <- coda.samples(model = diam_mod1, 
                      variable.names = c("mu"), 
                      n.iter = 5000)
...
gelman.diag(samps_mod1_5k)

# When did it converge? 
GBR_1k <- gelman.plot(samps_mod1_1k)
GBR_5k <- gelman.plot(samps_mod1_5k)
...
# Summary stats
summary(mod1_post_burnin[,,2])
```

Posterior estimate of the mean: 
Mean diameter of 16.57 with a SD of 0.3 and time-series SE of 0.005. The model converged, as the parameter density and MCMC "history" plots show overlapping chains that remain consistent throughout the iterations, with random deviations that are approximately normally distributed about the mean. 

This approach used 3 chains and a burn-in value of 1,000. The ESS was 3157. The model converged after very few iterations. This is reflected in the plots of the MCMC "history"; convergence is achieved when the chains are comparable and practically indistinguishable. Because of this, the model did not need more iterations. 
