...
inits_task1 <- list() 
inits_task1[[1]] <- list(mu = 15)
inits_task1[[2]] <- list(mu = 20)
inits_task1[[3]] <- list(mu = 25)
...
# Establish model
diam_mod1 <- jags.model(file = textConnection(NormalMeanN), 
                        data = data, 
                        inits = inits_task1, 
                        n.chains = chains)
...
# Sample posterior
# with 1000 samples
samps_mod1_1k <- coda.samples(model = diam_mod1, 
                      variable.names = c("mu"), 
                      n.iter = iterations)
...
gelman.diag(samps_mod1_1k)

# When did it converge? 
GBR_1k <- gelman.plot(samps_mod1_1k)

# First iteration after convergence 
burnin <- GBR_1k$last.iter[tail(which(GBR_1k$shrink[,,2] > 1.1),1)+1]

# What if no burn-in? 
if (length(burnin) == 0) burnin = 1

# Post-burn-in diagnostics
mod1_post_burnin <- window(samps_mod1_1k, start=burnin)
mod1_results <- c(mod1_post_burnin[[1]], mod1_post_burnin[[2]], mod1_post_burnin[[3]]) # samples from all chains
summary(mod1_post_burnin)
gelman.diag(mod1_post_burnin)
plot(mod1_post_burnin)

# Autocorrelation plot 
acfplot(mod1_post_burnin)

# Effective sample size
effectiveSize(mod1_post_burnin)

# Plot evolution of sample quantiles as a function of number of iterations
cumuplot(mod1_post_burnin, probs=c(0.025,0.25,0.5,0.75,0.975))
...
Mean diameter of 16.56 with a SD of 0.3 and time-series SE of 0.005. The model converged, as the parameter density and MCMC "history" plots show overlapping chains that remain consistent throughout the iterations, with random deviations that are approximately normally distributed about the mean. 

This approach used 3 chains and a burn-in value of 1,000. The ESS was 3000. The model converged after very few iterations. This is reflected in the plots of the MCMC "history"; convergence is achieved when the chains are comparable and practically indistinguishable. Because of this, the model did not need more iterations. Didn't thin the model for this.
...
model {
  mu ~ dnorm(mu0,T) # prior on the mean 
  S ~ dnorm(1/25, 1/15) # prior on the variance
  for(i in 1:N){
    X[i] ~ dnorm(mu,S) # data model
  }
}
...
inits_task2 <- list() 
inits_task2[[1]] <- list(mu = 15, S = 1/10)
inits_task2[[2]] <- list(mu = 20, S = 1/30)
inits_task2[[3]] <- list(mu = 25, S = 1/50)
...
# Establish model
diam_mod2 <- jags.model(file = textConnection(NormalMeanN_uncert), 
                        data = data_task2, 
                        inits = inits_task2, 
                        n.chains = chains_2)

# Sample posterior
samps_mod2_5k <- coda.samples(model = diam_mod2, 
                      variable.names = c("mu", "S"), 
                      n.iter = iterations_2)

plot(samps_mod2_5k)

gelman.diag(samps_mod2_5k)

# First iteration after convergence 
GBR_2_5k <- gelman.plot(samps_mod2_5k)

burnin_mod2 <- GBR_2_5k$last.iter[max(tail(which(GBR_2_5k$shrink[,1,2] > 1.1), 1))+1]
# What if no burn-in? 
if (length(burnin_mod2) == 1 & is.na(burnin_mod2[1])) burnin_mod2 = 1


# Post-burn-in diagnostics
mod2_post_burnin <- window(samps_mod2_5k, start=burnin_mod2)
mod2_results <- c(mod2_post_burnin[[1]], mod2_post_burnin[[2]], mod2_post_burnin[[3]]) # samples from all chains
summary(mod2_post_burnin)
gelman.diag(mod2_post_burnin)
plot(mod2_post_burnin)

# Autocorrelation plot 
acfplot(mod2_post_burnin)

# Effective sample size
effectiveSize(mod2_post_burnin)

# Plot evolution of sample quantiles as a function of number of iterations
cumuplot(mod2_post_burnin,probs=c(0.025,0.25,0.5,0.75,0.975))

# Summary stats
summary(mod2_post_burnin[,,2])
```

< will update with written answers for Task 2 > 
